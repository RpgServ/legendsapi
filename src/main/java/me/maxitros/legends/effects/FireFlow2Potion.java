...
public class FireFlow2Potion extends AdvancedModPotion {

    protected FireFlow2Potion(boolean isBadEffectIn, int liquidColorIn) {
    ...
    public void performModEffect(AdvancedModPotionEffect effect, EntityLivingBase entityLivingBaseIn, int amplifier, int duration, float damage) {
        if (duration > 0) {
            ...
            {
                effect.target.attackEntityFrom(DamageSource.MAGIC, damage*((25f + 5f*(amplifier-1))/100f));
                effect.target.addPotionEffect(new ModPotionEffect(PotionRegistry.FireFlowResultPotion, 20*(3 + amplifier), amplifier, damage));
                effect.target = null;
            }
