...
public class LightingFlow2Potion extends AdvancedModPotion {

    protected LightingFlow2Potion(boolean isBadEffectIn, int liquidColorIn) {
    ...
    public void performModEffect(AdvancedModPotionEffect effect, EntityLivingBase entityLivingBaseIn, int amplifier, int duration, float damage) {
        if (duration > 0) {
            ...
            {
                effect.target.attackEntityFrom(DamageSource.GENERIC, damage*((50f + 5f*(amplifier-1))/100f));
                effect.target.addPotionEffect(new ModPotionEffect(PotionRegistry.LightningResultPotion, 20*(2 + amplifier), amplifier, damage));
                effect.target = null;
            }
