...
public class IceFlowPotion extends AdvancedModPotion {

    protected IceFlowPotion(boolean isBadEffectIn, int liquidColorIn) {
    ...
    public void performModEffect(AdvancedModPotionEffect effect, EntityLivingBase entityLivingBaseIn, int amplifier, int duration, float damage) {
        if (duration > 0) {
            ...
            if(duration%20==0)
            {
                ...
                {
                    entityLivingBase.attackEntityFrom(DamageSource.GENERIC, damage*((25f + 5f*(amplifier-1))/100f));
                    entityLivingBase.addPotionEffect(new ModPotionEffect(PotionRegistry.IceStunResultPotion, 20*(5 + amplifier), amplifier, damage));
                }
